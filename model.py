...
def symmetric_mzi_matrix(phs_layer, phase_first=False):
    ...
    for i in range(phs_layer.shape[0] // 2):
        ...
        # phase_first=True builds D @ H; otherwise build H @ D.
        matrix = mat3 @ base_mat if phase_first else base_mat @ mat3
...
def mzi_mesh(global_phi, parallel=8):
    ...
    for lay_idx in range(global_phi.shape[0]):
        ...
        # The first stage is H D; subsequent stages are D H, giving
        # H D (D H)^(N-1). For ten layers this is H D D H ... D H.
        phase_first = lay_idx > 0
        if lay_idx in [2, 4, 6, 8]:
            matrix_layer = symmetric_mzi_matrix(layer_phi[1:-1], phase_first=phase_first)
            # padding 两端用 [1]
            ...
            matrix_layer = symmetric_mzi_matrix(layer_phi, phase_first=phase_first)
            matrix_mesh = matrix_mesh @ matrix_layer
    return matrix_mesh
...
class SimMZIMitrix(torch.nn.Module, MZIMatrix):
    ...
    def forward(self, x):
        ...
        # Gray values are used directly as optical amplitudes.
...
class TiledMZIMatrix(nn.Module, MZIMatrix):
    ...
    def forward(self, x):
        ...
        # Gray values are used directly as optical amplitudes, as in TrainONN-16.
        x = x.to(dtype=torch.complex64)
        ...
        # 先对每个输入块单独探测光强，再在输入块维度非相干求和。
        out_intensity = out_complex.abs().square().sum(dim=1)

        out_flat = out_intensity.view(batch_size, -1)
...
class PPOFixedMZI(LightningModule):
    ...
    def on_test_epoch_start(self):
        ...
        self._apply_policy_mean_to_env()
        self._test_confusion_matrix = torch.zeros(
            (self.env.out_features, self.env.out_features),
            dtype=torch.long,
            device=self.device,
        )
    ...
    def test_step(self, batch_data, batch_idx):
        ...
        y_target_batch = y_target_batch.to(self.device)
        predictions = self.env.step(x_batch).argmax(dim=1)

        flat_indices = y_target_batch * self.env.out_features + predictions
        self._test_confusion_matrix += torch.bincount(
            flat_indices,
            minlength=self.env.out_features * self.env.out_features,
        ).reshape(self.env.out_features, self.env.out_features)

        batch_acc = (predictions == y_target_batch).float().mean()
        self.log(
            "test_acc",
            batch_acc,
            on_step=False,
    ...
    def on_test_epoch_end(self):
        confusion_matrix = self._test_confusion_matrix.detach().cpu()
        print("\nTest confusion matrix (rows=true labels, columns=predicted labels):")
        ...
        axis.set(
            ...
            xticks=range(self.env.out_features),
            yticks=range(self.env.out_features),
        )
        threshold = confusion_matrix.max().item() / 2
        for row in range(self.env.out_features):
            for column in range(self.env.out_features):
...
#& 'D:\PycharmProjects\pythonProject\.venv\Scripts\mlflow.exe' server --backend-store-uri 'sqlite:///D:/TrainONN-Test/TrainONN-Test/mlflow.db' --port 5000
